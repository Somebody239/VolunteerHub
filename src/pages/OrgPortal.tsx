...
import { Building2, ShieldCheck, Clock3, Users, BarChart3, Plus, CheckCircle2, AlertTriangle, CalendarDays, Filter, Edit2, Trash2 } from "lucide-react";
...
const OrgPortal: React.FC = () => {
  ...
  const listQuery = useQuery({
    ...
    queryFn: async (): Promise<Opportunity[]> => {
      ...
      const { data, error } = await supabase
        .from("opportunities")
        .select("id, organizer_id, title, description, category, location, start_dt, end_dt, slots, created_at, fcfs, tags, is_deleted, application_form, min_age, max_age, apply_url, contact_email")
  ...
  const createMutation = useMutation({
    ...
    onError: (e: any) => toast({ title: editingId ? "Could not update" : "Could not create", description: e.message ?? String(e), variant: "destructive" }),
  });

  const updateMutation = createMutation; // Alias for clarity
  ...
  const deleteMutation = useMutation({
    ...
    onError: (e: any) => toast({ title: "Could not delete", description: e.message ?? String(e), variant: "destructive" }),
  });
  ...
  const jobsByTab = useMemo(() => {
    ...
    // Treat anything not past as active (includes upcoming)
    const isActive = (o: Opportunity) => !isPast(o);
    return {
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    ...
                    <Button 
                      ...
                      onClick={() => {
                        ...
                        const opp = o as any; // Cast to access all fields
                        setEditingId(o.id);
                        form.reset({
                          title: o.title,
                          ...
                          category: o.category,
                          location: o.location || "",
                          min_age: opp.min_age ?? undefined,
                          max_age: opp.max_age ?? undefined,
                          start_dt: o.start_dt ? new Date(o.start_dt).toISOString().slice(0, 16) : "",
                          end_dt: o.end_dt ? new Date(o.end_dt).toISOString().slice(0, 16) : "",
                          slots: o.slots ?? 10,
                          fcfs: o.fcfs ?? false,
                          tagsCsv: (o.tags ?? []).join(", "),
                        ...
                        setQuestions((opp.application_form ?? []) as any[]);
                        // Scroll to form
                        document.getElementById('create-opportunity')?.scrollIntoView({ behavior: 'smooth' });
                        toast({ title: "Edit mode", description: "Update the form below and click 'Update' to save changes" });
                      }}
                    >
                      Edit
                    </Button>
                    <Button 
                      ...
                      onClick={async () => {
                        if (!window.confirm(`Are you sure you want to delete "${o.title}"? This action cannot be undone.`)) return;
        ...
        <section id="create-opportunity" className="glass-card p-4 md:p-6 rounded-xl">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit((v) => createMutation.mutate(v))} className="grid gap-3 md:grid-cols-2">
              ...
              {/* Application Form Builder (inline) */}
              <div className="md:col-span-2 border-t pt-3 mt-2 space-y-2">
                <h3 className="text-sm font-semibold mb-1">Application Form (optional)</h3>
                <div className="grid md:grid-cols-4 gap-2 items-end">
                  ...
                  <div className="flex items-center gap-2">
                    <input id="req_org" type="checkbox" checked={newQ.required} onChange={(e) => setNewQ((q) => ({ ...q, required: e.target.checked }))} />
                    <label htmlFor="req_org" className="text-xs text-muted-foreground">Required</label>
                  </div>
                  {newQ.type === "select" && (
                ...
                {editingId && (
                  <Button 
                    ...
                    onClick={() => {
                      ...
                      form.reset({ title: "", description: "", category: "Community", location: "", min_age: undefined, max_age: undefined, start_dt: "", end_dt: "", slots: 10, fcfs: false, tagsCsv: "", apply_url: "", contact_email: "" });
                      setQuestions([]);
                      toast({ title: "Edit cancelled", description: "Switched back to create mode" });
                    }}
                  >
                    Cancel Edit
                  </Button>
