...
const Index = () => {
  ...
  const opportunitiesQuery = useQuery({
    ...
    queryFn: async (): Promise<any[]> => {
      const { data, error } = await supabase
        .from("opportunities")
        .select(
          "id, organizer_id, title, description, category, location, start_dt, end_dt, slots, tags, apply_url, contact_email, is_deleted",
        )
        .eq("is_deleted", false)
    ...
    staleTime: 2 * 60 * 1000, // 2 minutes - opportunities change less frequently
  });
  ...
  const filtered = useMemo(() => {
    ...
    const list = (opportunitiesQuery.data ?? [])
      ...
      .map<JobDetails & { opportunityId: string; tags?: string[] }>((o) => ({
        title: o.title,
  ...
  const computeScoreAndXp = (item: any): { score: number; xp: number; stars: number } => {
    ...
    if (item.duration) {
      const m = String(item.duration).match(/(\d+)(?=\s*hour)/i);
    ...
    const stars = Math.max(0, Math.min(5, Math.round((score || 0) / 10)));
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-6 space-y-6">
        ...
        <section>
          ...
          <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 auto-rows-fr items-stretch">
            {(volunteerOpportunitiesQuery.data?.results ?? [])
              .slice(0, 6)
              .map((item, index) => {
            ...
            {!volunteerOpportunitiesQuery.isLoading && (volunteerOpportunitiesQuery.data?.results ?? []).length === 0 && (
              <div className="text-sm text-muted-foreground">
        ...
        <JobDetailsDialog
          ...
          onApply={async (id, answers) => {
            ...
            // Check if this is an external opportunity
            if (selected.opportunityId && selected.opportunityId.startsWith('ext-')) {
              ...
              // Handle regular database application
        ...
        <section>
          ...
          <div className="grid gap-3">
            ...
            {/* Internal applications - Consistent styling (only active ones) */}
            {applicationsQuery.data?.filter(a => !['declined', 'withdrawn'].includes(a.status)).map((a) => {
              ...
              const statusConfig = {
                applied: { color: "text-yellow-300", bg: "bg-yellow-900/30", dot: "bg-yellow-500", border: "border-yellow-600/50", glow: "shadow-yellow-500/30" },
                accepted: { color: "text-green-300", bg: "bg-green-900/30", dot: "bg-green-500", border: "border-green-600/50", glow: "shadow-green-500/30" },
                declined: { color: "text-red-300", bg: "bg-red-900/30", dot: "bg-red-500", border: "border-red-600/50", glow: "shadow-red-500/30" },
                withdrawn: { color: "text-gray-300", bg: "bg-gray-900/30", dot: "bg-gray-500", border: "border-gray-600/50", glow: "shadow-gray-500/30" },
                waitlisted: { color: "text-blue-300", bg: "bg-blue-900/30", dot: "bg-blue-500", border: "border-blue-600/50", glow: "shadow-blue-500/30" },
                done: { color: "text-blue-300", bg: "bg-blue-900/30", dot: "bg-blue-500", border: "border-blue-600/50", glow: "shadow-blue-500/30" },
                verify: { color: "text-amber-300", bg: "bg-amber-900/30", dot: "bg-amber-500", border: "border-amber-600/50", glow: "shadow-amber-500/30" }
              };
              ...
              return (
                <div key={a.id} className="glass-card p-4 hover:shadow-lg transition-all duration-200">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      <div className={`w-3 h-3 rounded-full ${config.dot} flex-shrink-0`}></div>
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="text-xs text-muted-foreground">
                          Applied: {a.created_at ? new Date(a.created_at).toLocaleDateString() : 'Unknown'}
                  </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 flex-shrink-0">
