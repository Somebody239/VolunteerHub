  ...
  useEffect(() => {
    const handle = setTimeout(async () => {
      ...
      try {
        ...
        const mapped: LocationValue[] = (data || []).map((d: { display_name?: string; lat?: string; lon?: string }) => ({ label: d.display_name || "", lat: parseFloat(d.lat || "0"), lon: parseFloat(d.lon || "0") }));
        setResults(mapped);
      } catch {
        // Ignore API errors for location search
        setResults([]);
      }
    }, 250);
