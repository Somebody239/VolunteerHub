...
export const ScheduleEditor: React.FC<Props> = ({ value, onChange }) => {
  ...
  const applyToAllWeekdays = (slot: { start: string; end: string }) => {
    const next: WeeklySchedule = { ...sched };
    (['mon','tue','wed','thu','fri'] as const).forEach((k) => {
      next[k] = Array.from(new Set([...(next[k] ?? []), slot].map(s => `${s.start}-${s.end}`)))
        .map(key => ({ start: key.split('-')[0]!, end: key.split('-')[1]! }));
    });
    void commit(next);
  };
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2">
        {presets.map(p => (
          <button key={p.label} type="button" className="text-xs px-2 py-1 rounded border border-border hover:bg-muted/40" onClick={() => applyToAllWeekdays({ start: p.start, end: p.end })}>{p.label}</button>
        ))}
      </div>
      {days.map(({ key, label }) => (
